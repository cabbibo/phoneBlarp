...
public class TouchBlarp : Game
{
  ...
  public float blarpStartDrag;
  public float blarpMinDrag;
  public int blarpMinDragScore;


  public float blarpStartForce;
  public float blarpMinForce;
  public int blarpMinForceScore;
  ...
  public float BlarpDrag()
  {
    float v1 = blarpStartDrag - blarpMinDrag;
    float v2 = (float)score / (float)blarpMinDragScore;
    float fDrag = blarpStartDrag - v1 * v2;
    fDrag = Mathf.Clamp(fDrag, blarpMinDrag, blarpStartDrag);
    return fDrag;
  }

  public float BlarpForce()
  {
    float v1 = blarpStartForce - blarpMinForce;
    float v2 = (float)score / (float)blarpMinForceScore;
    float fForce = blarpStartForce - v1 * v2;
    fForce = Mathf.Clamp(fForce, blarpMinForce, blarpStartForce);
    return fForce;
  }
  ...
  void Update()
  {
    ...
    if (Input.GetMouseButton(0) && touchDown)
    {
      ...
      if (collider.Raycast(ray, out hit, 100.0f))
      {
        ...
        blarpRigidBody.AddForce(Vector3.Scale((hit.point - blarp.transform.position), new Vector3(1, 0, 1)) * 1 * BlarpForce() * Time.deltaTime * 60);
  ...
  public override void DoRestart()
  {
    ...
    blarpRigidBody.drag = blarpStartDrag;
    ...
    enemyCollect = 0;
    walls.EnemyCollect(enemyCollect);
  ...
  public void TailGrow()
  {
    tailSize++;
    enemyCollect -= 1;
    enemyCollect = Mathf.Max(0, enemyCollect);
    walls.EnemyCollect(enemyCollect);
  ...
  private string appStoreLink = "https://cabbibo.itch.io/blarp2d";

  public void ShareToTwitter()
  {

    string nameParameter = "BLARP! YOUR! SCREEN!";//this is limited in text length 
  ...
  {
    Application.OpenURL("https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fcabbibo.itch.io%2Fblarp2d");
  }
