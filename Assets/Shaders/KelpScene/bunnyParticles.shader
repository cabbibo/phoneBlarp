...
float3 refl = reflect( normalize(_WorldSpaceCameraPos-v.worldPos) , v.nor);
        float reflM = dot( refl , _WorldSpaceLightPos0 );
               ...
               // if( ( lookupVal + 1.3) - 1.2*length( tCol ) < .5 ){ discard;}
                ...
                float aVal = length(v.uv-.5)  * .07 + length(v.vel) * 5 + noise( v.worldPos * 1.5 + float3(0,0,_Time.y)) * .2;

                float4 audioColor = tex2D( _AudioMap , float2( aVal , 0 ) );
                col.xyz = normalize(v.vel ) * .5 + .5;
                col.xyz = length( v.vel) * 30;
                col = audioColor * 6;
                //col = noise( v.worldPos * 1.5 + float3(0,0,_Time.y));
                //if( v.uv.x >.83){ col = 1;}
                return col;
            }
